 ...
 bool soldier_round;
  ...
  void go_queen(Ant a){
    if(near_ally_queen(a.pos)){
      if(cell(a.pos).bonus != None) free_positions[a.pos.i][a.pos.j] = false;
      else {
        leave(a.id);
        return;
      }
  ...
  void move_workers(){
    ...
    for(int id : W){
      Ant a = ant(id);
      if(avoid_enemies(a));
      else a.bonus == None ? search_food(a) : go_queen(a);
    }
  }
  ...
  bool avoid_soldiers_queens(Ant a){
    if(near_enemy_soldier(a.pos) or near_enemy_queen(a.pos)){
      for(int k=0; k<4; k++){
        Pos adj = a.pos + Dir(k);
        if(pos_ok(adj) and free_positions[adj.i][adj.j] and cell(adj).type != Water and !near_enemy_soldier(adj) and !near_enemy_queen(adj)){
          move(a.id,Dir(k));
          return true;
        }
      }
  ...
  bool lay_solder_egg(Ant a){
    if((round() >= 70 and round() <= 80) or (round() >= 120 and round() <= 130)){
      soldier_round = true;
  ...
  void move_queen(){
    ...
    for(int id : Q){
      ...
      else if(!soldier_round and lay_worker_egg(a)) return;
      else if(w == 0) search_food_queen(a);
    }
  }
